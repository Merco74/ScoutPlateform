...
// Configuration de multer (stockage temporaire avant upload Cloudinary)
...
app.post('/inscription', upload.fields([
  ...
  try {
    ...
    const uploadToCloudinary = async (file, folder) => {
      ...
      return new Promise((resolve, reject) => {
        cloudinary.uploader.upload_stream(
          { resource_type: 'raw', folder: `scouts-cluses/${folder}`, public_id: `${Date.now()}_${file.originalname}` },
          (error, result) => {
    ...
    formData.vaccinScan = files.vaccinScan ? await uploadToCloudinary(files.vaccinScan[0], 'uploads') : '';
    formData.medicationScan = files.medicationScan ? await uploadToCloudinary(files.medicationScan[0], 'uploads') : '';
    formData.recommendationMedicalScan = files.recommendationMedicalScan ? await uploadToCloudinary(files.recommendationMedicalScan[0], 'uploads') : '';
    formData.otherDocuments = files.otherDocuments ? await Promise.all(files.otherDocuments.map(file => uploadToCloudinary(file, 'uploads'))) : [];

    // Generate Authorization PDF
    const docAuth = new PDFDocument({ margin: 50, size: 'A4' });
    const authPdfBuffers = [];
    docAuth.on('data', authPdfBuffers.push.bind(authPdfBuffers));
    docAuth.on('end', async () => {
      const pdfData = Buffer.concat(authPdfBuffers);
      const authPdfUrl = await new Promise((resolve, reject) => {
        cloudinary.uploader.upload_stream(
          { resource_type: 'raw', folder: 'scouts-cluses/pdfs', public_id: `autorisation_${formData.nom}_${Date.now()}.pdf` },
          (error, result) => {
            if (error) return reject(error);
            resolve(result.secure_url);
          }
      ...
      const sanitaryPdfBuffers = [];
      docSanitary.on('data', sanitaryPdfBuffers.push.bind(sanitaryPdfBuffers));
      docSanitary.on('end', async () => {
        const sanitaryPdfData = Buffer.concat(sanitaryPdfBuffers);
        const sanitaryPdfUrl = await new Promise((resolve, reject) => {
          cloudinary.uploader.upload_stream(
            { resource_type: 'raw', folder: 'scouts-cluses/pdfs', public_id: `fiche_sanitaire_${formData.nom}_${Date.now()}.pdf` },
            (error, result) => {
              if (error) return reject(error);
              resolve(result.secure_url);
            }
        ...
        res.json({ message: 'Inscription enregistrée avec succès !', pdfUrl: authPdfUrl, sanitaryPdfUrl: sanitaryPdfUrl });
      });

      // Authorization PDF content
